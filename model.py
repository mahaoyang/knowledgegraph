...
def model():
    ...
    p_encoder = layers.Bidirectional(layers.LSTM(int(MAX_WORD_INDEX / 2), return_sequences=True))(passage)
    p_encoder = layers.Bidirectional(layers.LSTM(int(MAX_WORD_INDEX / 2), return_sequences=True))(p_encoder)

    p_encoder = layers.LSTM(MAX_WORD_INDEX, return_sequences=True)(p_encoder)
    p_encoder = layers.LSTM(MAX_WORD_INDEX, return_sequences=True)(p_encoder)
    # p_encoder = passage
